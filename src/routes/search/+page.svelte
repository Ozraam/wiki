...
<main>
    ...
    <button onclick={toogleTagPanel} class="mb-4 flex items-center gap-2 cursor-pointer active:bg-gray-200 px-1">
        Tag Filter
    </button>

    <aside
        class="flex flex-col gap-1 bg-white fixed h-full w-full transition-transform top-0 left-0 z-20 p-4 duration-500 {tagPanelOpen ? 'translate-x-0' : 'translate-x-full'}"
    >
        <h3 class="text-lg font-bold mb-2">Filter by Tags</h3>
        {#each tags as tag}
            <span>
                <input type="checkbox" id={tag.tag} value={tag} class="" bind:checked={tag.checked} />
                <label for={tag.tag} class="text-gray-700">{tag.tag}</label>
            </span>
        {/each}
        <div class="flex justify-between mt-4">
            <button onclick="{clearTags}" class="active:bg-gray-200 cursor-pointer flex items-center gap-2">
                <iconify-icon icon="material-symbols:close" class="text-red-500"
                ></iconify-icon>
                <span class="text-gray-700">Clear Filters</span>
            </button>
            <button onclick="{allTags}" class="active:bg-gray-200 cursor-pointer flex items-center gap-2">
                <iconify-icon icon="material-symbols:check" class="text-green-500"></iconify-icon>
                <span class="text-gray-700">Select All</span>
            </button>
            <button onclick="{toogleTagPanel}" class="active:bg-gray-200 cursor-pointer flex items-center gap-2">
                <iconify-icon
